...
def extract_features(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    pd.DataFrame
        ...
        and each column corresponds to a behavioral feature
    """

    features = []

    # Predefined list of event types to ensure a fixed feature dimension
    action_list = [
    ...
    # Select the top 5 most frequently used devices across the whole dataset
    top_devices = dataframe['device_id'].value_counts().nlargest(5).index
...
# Exclude user_id column and normalize all numeric features
...
plt.title('User Behavior Similarity (Cosine)')
plt.tight_layout()
